# -*- coding: utf-8 -*-
"""
wizard/ec_iva_wizard.py
=======================
Wizard del Formulario 104 para Odoo 17 Community.

Flujo:
  1. Usuario selecciona período + crédito mes anterior + formato
  2. Wizard llama a ec.iva.engine.calcular_104()
  3. Según el formato elegido: genera PDF (QWeb), XLSX o XML (DIMM)
...
class EcIvaWizard(models.TransientModel):
    _name = 'ec.iva.wizard'
    _description = 'Wizard Formulario 104 IVA Ecuador - Community'
    ...
    def action_generar(self):
        ...
        resultado = self._compute_resultado()
        self.write({
            'vista_previa_html': self._build_preview_html(resultado),
            'estado': '✓ Reporte generado. Puedes visualizarlo en pantalla y descargar el archivo.',
    ...
    def action_exportar_pdf(self):
        ...
        return self._generar_pdf(resultado, mes_str, anio_str)
    ...
    def action_exportar_xlsx(self):
        ...
        mes_str = date_from[5:7]
        anio_str = self._to_plain_year(date_from[:4])
        return self._generar_xlsx(resultado, mes_str, anio_str)
    ...
    def action_exportar_xml(self):
        ...
        mes_str = date_from[5:7]
        anio_str = self._to_plain_year(date_from[:4])
        return self._generar_xml(resultado, mes_str, anio_str)

    # ────────────────────────────────────────────────────────────
    # PDF via QWeb (Community nativo)
    # ────────────────────────────────────────────────────────────

    def _generar_pdf(self, resultado, mes_str, anio_str):
        """
        Genera el PDF usando ir.actions.report (QWeb).
        Pasa los datos calculados al template via data dict.
        """
        # Guardar resultado en el wizard para que QWeb lo acceda
        # (truco Community: usamos el wizard como contexto del reporte)
        self.write({'archivo_nombre': f'F104_{anio_str}{mes_str}.pdf'})

        # Llamar al reporte QWeb pasando el wizard como record
        # y los datos del 104 en el contexto
        return self.env.ref('l10n_ec_iva_report.action_ec_iva_104_pdf').with_context(
            resultado_104=resultado
        ).report_action(self)
    ...
    def _generar_xlsx(self, resultado, mes_str, anio_str):
        try:
            import xlsxwriter
        except ImportError:
        ...
        row = 0
        ws.merge_range(row, 0, row, 3, 'FORMULARIO 104 — DECLARACIÓN DE IVA', f_title)
        ws.set_row(row, 22)
        row += 1
        ...
        # Encabezado empresa
        for label, val in [('RUC:', meta['ruc']), ('Razón Social:', meta['razon_social']),
        ...
        for i, h in enumerate(['Casillero', 'Descripción', 'Cuentas relacionadas', 'Valor (USD)']):
        ...
        filename = f'F104_{anio_str}{mes_str}.xlsx'
        self.write({
            'archivo_datos': base64.b64encode(xlsx_bytes),
    ...
    def _generar_xml(self, resultado, mes_str, anio_str):
        casilleros = resultado['casilleros']
        meta = resultado['meta']
        ...
        filename = f'F104_{anio_str}{mes_str}.xml'
        self.write({
            'archivo_datos': base64.b64encode(pretty if isinstance(pretty, bytes) else pretty.encode()),
            'archivo_nombre': filename,
        })
        return self._reopen_wizard()
    ...
    def _reopen_wizard(self):
        return {
            ...
            'res_model': 'ec.iva.wizard',
            'res_id': self.id,
            'view_mode': 'form',
            'view_id': self.env.ref('l10n_ec_iva_report.view_ec_iva_wizard_result_form').id,
            'target': 'new',
        }
    ...
    def _build_preview_html(self, resultado):
        """Construye una vista previa en pantalla del formulario 104."""
        ...
        return (
            ...
            +
            "<p><i>El archivo PDF/XLSX/XML sigue disponible para descarga según formato seleccionado.</i></p>"
            "</div>"
        )
